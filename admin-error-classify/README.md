...
## โหมด "ask" — สรุป/ถามตอบข้อมูลอิสระ (ใช้ใน clinic.html)

นอกจากโหมดจัดหมวด (`items`/`categories`) worker นี้รับโหมดที่สองคือ `mode:'ask'` สำหรับ
ให้ AI สรุปหรือตอบคำถามอิสระจากข้อมูลสรุป/สถิติที่ส่งไป (ไม่ควรมี HN/ชื่อผู้ป่วยติดไปด้วย):

```
POST { mode: "ask", question: "สรุปภาพรวม...", data: {...ตัวเลขสรุป...}, context: "..." }
→ { answer: "..." }
```

ใช้ `X-App-Token` และโควตารายวันร่วมกับโหมดจัดหมวด (นับรวมกัน 300 ครั้ง/วัน) ไม่ต้องตั้งค่า secret เพิ่ม
